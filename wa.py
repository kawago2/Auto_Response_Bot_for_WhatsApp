...
def after_end_message():
    global x, y

    position = pt.locateOnScreen("bar_top.png", confidence=.6)
    x = position[0]
    y = position[1]
    pt.moveTo(x, y+80, duration=.5)
    pt.click()
...
def process_response(message):
    ...
    if str(message).lower() == 'leb' or 'kaleb':
        return "maaf kaleb sedang afk.\n_pesan ini dikirim otomatis mohon menunggu !!_"
